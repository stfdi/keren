...
<body>
    ...
    if (isset($_GET['status'])) {
        $id = $_GET['status'];
        $sql = "UPDATE todolist SET status = 'selesai' WHERE produkid = $id";
        $query = mysqli_query($conn, $sql);
    }
    ...
    if (isset($_GET['hapus'])) {
        $id = $_GET['hapus'];
        $sql = "DELETE FROM todolist WHERE produkid = $id";
        $query = mysqli_query($conn, $sql);
    }
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr><td>" . $row["produkid"] . "</td><td>" . $row["kegiatan"] . "</td><td>" . $row["status"] . "</td><td>";
            echo "<a href='" . $_SERVER['PHP_SELF'] . "?selesai=" . $row["produkid"] . "'>Selesai</a> ";
            echo "<a href='" . $_SERVER['PHP_SELF'] . "?hapus=" . $row["produkid"] . "'>Hapus</a>";
            echo "</td></tr>";
        }
        echo "</tbody></table>";
    } else {
